...
class GetBaidu:
    ...
    def getList(self):
        ...
        for a in retList:
            ...
            if realUrl.endswith('/'):
                try:
                    webUrlGet = requests.get(realUrl,headers=HTTP_HEADERS,timeout=HTTP_TIMEOUT)
                ...
                # 验证sdk是否完整
                tidT = webUrlGetHtml.xpath("//title") 
                tidD = webUrlGetHtml.xpath("//meta[@name='description']/@content")
                tidK = webUrlGetHtml.xpath("//meta[@name='keywords']/@content")
                # print(tidT,tidD,tidK);
                # exit()
                if  len(tidT) & len(tidD) & len(tidK) :
                    oneList[3] = "完整"
                else:
                    oneList[3] = "不完整"
                csone = tuple(oneList)
